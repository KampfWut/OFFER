...
def build_deepwalk_corpus(G,
    ...
    for cnt in range(num_paths):
        ...
        for node in nodes:
            walks.append(
                G.random_walk(path_length, rand=rand, alpha=alpha, start=node))
...
def build_deepwalk_corpus_iter(G,
    ...
    for cnt in range(num_paths):
        rand.shuffle(nodes)
        for node in nodes:
            yield G.random_walk(path_length,
                                rand=rand,
                                alpha=alpha,
                                start=node)
...
def load_edgelist(graph_type, undirected=True):
    G = Graph()
    for x in graph_type.nodes():
        if len(list(graph_type.neighbors(x))) == 0:
            G[x].append(x)
        else:
            for y in list(graph_type.neighbors(x)):
                G[x].append(y)
                if undirected:
                    G[y].append(x)
