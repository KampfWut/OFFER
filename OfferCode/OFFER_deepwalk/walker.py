...
class BasicWalker:
        ...
        while len(walk) < walk_length:
            ...
            if len(cur_nbrs) > 0:
                for node in cur_nbrs:
                    prob.append(G[cur][node]['weight'])
                problist = [float(c) / float(sum(prob)) for c in prob]
                p = np.array(problist)
                next_step = np.random.choice(cur_nbrs, p=p.ravel())
    ...
    def simulate_walks(self, num_walks, walk_length):
        ...
        for walk_iter in range(num_walks):
            ...
            for node in nodes:
                new_length = (G.nodes[node]['modegree'] - 1.0) + walk_length
                # walks.append(pool.apply_async(deepwalk_walk_wrapper, (self, walk_length, node, )))
                walks.append(
                    self.deepwalk_walk(walk_length=new_length,
                                       start_node=node))
            # pool.close()
            # pool.join()
        # print(len(walks))
