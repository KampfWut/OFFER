...
class _LINE(object):
        ...
        while start_index < data_size:
            if mod == 0:
                ...
                for i in range(start_index, end_index):
                    if not random.random() < self.edge_prob[
                            shuffle_indices[i]]:
                ...
                for i in range(len(h)):
                    t.append(self.sampling_table[random.randint(
                        0, table_size - 1)])
    ...
    def gen_sampling_table(self):
        ...
        power = 0.75
        numNodes = self.node_size
        node_dic = self.g.read_node_label('.\\Dataset\\3_node_result.csv')
        print("Pre-procesing for non-uniform negative sampling!")
        node_degree = np.zeros(numNodes)  # out degree

        look_up = self.g.look_up_dict
        for edge in self.g.G.edges():
            node_degree[look_up[edge[0]]] = node_degree[look_up[edge[0]]] + (
                self.g.G[edge[0]][edge[1]]["weight"] *
                float(node_dic.get(edge[1])))
            node_degree[look_up[edge[1]]] = node_degree[look_up[edge[1]]] + (
                self.g.G[edge[1]][edge[0]]["weight"] *
                float(node_dic.get(edge[0])))
        ...
        total_sum = sum([
            self.g.G[edge[0]][edge[1]]["weight"] *
            (node_dic.get(edge[0]) + node_dic.get(edge[1]))
            for edge in self.g.G.edges()
        ])
        norm_prob = [
            self.g.G[edge[0]][edge[1]]["weight"] *
            (node_dic.get(edge[0]) + node_dic.get(edge[1])) * data_size /
            total_sum for edge in self.g.G.edges()
        ]
        num_small_block = 0
        num_large_block = 0
        cur_small_block = 0
...
class LINE(object):
    def __init__(self,
        ...
        self.vectors = {}
        if order == 3:
            self.model1 = _LINE(graph,
                                rep_size / 2,
                                batch_size,
                                negative_ratio,
                                order=1)
            self.model2 = _LINE(graph,
                                rep_size / 2,
                                batch_size,
                                negative_ratio,
                                order=2)
